...
describe("ecosystem-ci advanced features", () => {
  test("applies patchFiles and function actions before commands", async () => {
    ...
    try {
        ...
        ecosystem: [
          {
            ...
            patchFiles: {
              "patched.txt": (content) => content.replace("original", "patched"),
            },
            actions: [
              () => {
                functionRuns += 1;
                const patchedContent = fs.readFileSync(path.join(pkgDir, "patched.txt"), "utf8");
      ...
      expect(updatedPackageJson.dependencies.monopkg).toBe("file:../../packages/cli");
